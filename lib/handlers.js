    ...
    _data.read("users", phone, (err, userData) => {
      ...
      if (!err && userData) {
        ...
        if (hashedPassword === userData.hashedPassword) {
          performance.mark("hashing password complete");
            ...
            if (userChecks.length < config.maxChecks) {
              ...
              dns.resolve(hostname, (err, records) => {
                if (!err && records) {
                  ...
                  callback(400, {
                    Error:
                      "The hostname has no DNS entries and therefore cannot resolve."
                  });
                }
              });
            } else {
                        ...
                        _data.update(
                          ...
                          err => {
                            if (!err) {
                              ...
                              callback(500, {
                                "Error:": "Could not update the user's checks."
                              });
                            }
                          }
                        );
